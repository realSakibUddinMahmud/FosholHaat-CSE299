...
import {
  ...
  type BuyerCheckoutSubmitResponse,
...
export default function BuyerCheckoutConfirmationPage() {
  ...
  async function submit() {
    const response = await apiPost<BuyerCheckoutSubmitResponse>(
      "/api/buyer/checkout/submit",
    );
      ...
      <div className={styles.formCard}>
        <div className={styles.reviewGrid}>
          <div className={styles.reviewTile}>
            ...
            <span>Ready for hub handoff.</span>
          </div>
          <div className={styles.reviewTile}>
            <strong>{copy.labels.paymentMethod}</strong>
            <span>Payment will be recorded with this order.</span>
          </div>
          {cart?.lines.some((line) => line.mode === "GROUP") ? (
            <div className={styles.reviewTileWide}>
              <strong>Group-buy pending rule</strong>
              <span>
                Checkout reserves your group quantity. Fulfillment starts only
                after the group target is filled.
              </span>
            </div>
          ) : null}
        </div>
