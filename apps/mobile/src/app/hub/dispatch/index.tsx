...
import { getHubDispatchCopy, type HubDispatchLoadStatus } from "@fosholhaat/types";
import { useStoredLocale } from "../../../lib/locale";
import { MOBILE_TOKENS, TOKENS } from "../../../styles/tokens";
import { HUB_DISPATCH_QUEUE } from "./_data";

function percentTone(value: number) {
  if (value >= 100) return styles.percentGood;
  if (value >= 80) return styles.percentWarm;
  return styles.percentCold;
}

export default function HubDispatchQueueScreen() {
  const router = useRouter();
  const { locale } = useStoredLocale();
  const copy = getHubDispatchCopy(locale);
  const [activeTab, setActiveTab] = useState<HubDispatchLoadStatus>(HUB_DISPATCH_QUEUE.activeTab);
  const loads = useMemo(() => HUB_DISPATCH_QUEUE.loads.filter((item) => item.status === activeTab), [activeTab]);
        ...
        <View style={styles.shell}>
          <View style={styles.header}>
            ...
            <View style={styles.headerPill}>
              <MaterialIcons name="local-shipping" size={16} color={TOKENS.brand.primary} />
              <Text style={styles.headerPillText}>{copy.activeLoading}</Text>
            </View>
          </View>

          <View style={styles.heroCard}>
            <Text style={styles.heroTitle}>{copy.hubTitle}</Text>
            <Text style={styles.heroSubtitle}>{copy.subtitle}</Text>
            <View style={styles.metricRow}>
              {(["staging", "ready", "departed"] as HubDispatchLoadStatus[]).map((status) => (
                <Pressable
                  key={status}
                ...
                >
                  ...
                  <Text style={styles.metricValue}>{HUB_DISPATCH_QUEUE.summary[status]}</Text>
                </Pressable>
              ))}
            </View>
          </View>
          ...
          {loads.length ? (
            <View style={styles.queueList}>
              {loads.map((load) => {
                const readiness = load.status === "departed" || load.status === "ready" ? 100 : 81;
                return (
                  <Pressable
                    key={load.loadId}
                    style={styles.queueCard}
                    onPress={() => router.push(`/hub/dispatch/${load.loadId}`)}
                  >
                    <View style={styles.queueCardHeader}>
                      ...
                      <View style={styles.statusPill}>
                        <Text style={styles.statusText}>{copy.statuses[load.status]}</Text>
                      </View>
                    </View>

                    <Text style={styles.destination}>{load.destination}</Text>
                    <Text style={styles.corridor}>{load.corridor}</Text>

                    <View style={styles.infoRow}>
                      <View style={styles.infoChip}>
                        <MaterialIcons name="inventory-2" size={15} color={TOKENS.brand.primary} />
                        <Text style={styles.infoChipText}>{load.parcelCount}</Text>
                      </View>
                      <View style={styles.infoChip}>
                        <MaterialIcons name="alt-route" size={15} color={TOKENS.color.textSecondary} />
                        <Text style={styles.infoChipText}>{load.stopCount}</Text>
                      </View>
                      <View style={[styles.infoChip, percentTone(readiness)]}>
                        <Text style={styles.infoChipText}>{copy.readiness} {readiness}%</Text>
                      </View>
                    </View>

                    <View style={styles.queueFooter}>
                      <Text style={styles.assignmentText}>{copy.assignmentStates[load.assignmentState]}</Text>
...
const styles = StyleSheet.create({
  ...
  brandTitle: { color: TOKENS.brand.primary, fontSize: MOBILE_TOKENS.font.sectionTitle.size, fontWeight: MOBILE_TOKENS.font.sectionTitle.weight, letterSpacing: 0 },
  ...
  loadId: { color: TOKENS.brand.primary, fontSize: 13, fontWeight: "800", textTransform: "uppercase" },
  ...
  percentGood: { backgroundColor: TOKENS.color.successSoft },
  percentWarm: { backgroundColor: TOKENS.color.soft },
  percentCold: { backgroundColor: TOKENS.color.surfaceMuted },
